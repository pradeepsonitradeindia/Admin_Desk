...
class FormlessField(Field):
    ...
    def formfield(self, **kwargs):
        raise TypeError("A Formless Field cannot be modified from Django Admin.")
...
class ListField(FormlessField):
    ...
    def get_db_prep_value(self, value, connection, prepared=False):
        if not isinstance(value, list):
            raise ValueError(f"Value: {value} must be of type list")
        return value
...
class ArrayModelField(Field):
    ...
    def get_db_prep_value(self, value, connection, prepared=False):
        ...
        if not isinstance(value, list):
            raise ValueError(
                "Expected value to be type list," f"Got type {type(value)} instead"
            )
