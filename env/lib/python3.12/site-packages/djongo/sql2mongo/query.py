...
class SelectQuery(Query):
    ...
    def __init__(self, *args):
        ...
        self.joins: typing.Optional[
            typing.List[typing.Union[InnerJoinConverter, OuterJoinConverter]]
        ] = []
    ...
    def _make_pipeline(self):
        ...
        if not (self.distinct or self.groupby) and self.selected_columns:
...
class InsertQuery(VoidQuery):
    ...
    def _columns(self, tok_id):
        ...
        if isinstance(tok[1], IdentifierList):
            self._cols = [
                SQLToken(an_id, None).column for an_id in tok[1].get_identifiers()
            ]
        else:
            ...
            if tok.match(tokens.Keyword, "VALUES"):
                ...
                if isinstance(tok[1], IdentifierList):
                    ...
                    if tok[1].match(tokens.Keyword, "DEFAULT"):
                        ...
                        self._vals.append((self.params[i] if i is not None else None,))
    ...
    def execute(self):
        ...
        for i, val in enumerate(self._vals):
            ...
            for fld, v in zip(self._cols, val):
                if auto and fld in auto["auto"]["field_names"] and v == self.DEFAULT:
                    continue
                ins[fld] = v
            docs.append(ins)
...
class AlterQuery(VoidQuery):
    ...
    def _drop_column(self):
        self.db_ref[self.left_table].update({}, {"$unset": {self._iden_name: ""}})
    ...
    def _add(self, tok_id):
        ...
        while tok_id is not None:
            if tok.match(
                ...
                tokens.Name.Builtin,
                ("integer", "bool", "char", "date", "datetime", "float", "time"),
            ):
                pass
            elif isinstance(tok, Identifier):
                self._iden_name = tok.get_name()
    ...
    def _index(self):
        self.db_ref[self.left_table].create_index(self.field_dir, name=self._iden_name)
        ...
        if tok.match(tokens.Keyword, "TABLE"):
            ...
            if isinstance(tok, Parenthesis):
                ...
                for col in tok.value.strip("()").split(","):
                    ...
                    _set[f"fields.{field}"] = {"type_code": type_code}
                ...
                if update:
                    self.db["__schema__"].update_one(
                        filter=_filter, update=update, upsert=True
                    )
