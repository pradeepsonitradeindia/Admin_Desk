...
class Converter:
    def __init__(
        self, query_ref: typing.Union["query.SelectQuery", "query.Query"], begin_id: int
    ):
...
class ColumnSelectConverter(Converter):
    def __init__(self, query_ref, begin_id):
        ...
        self.sql_tokens: typing.List[typing.Union[SQLToken, SQLFunc]] = []
...
class WhereConverter(Converter):
    ...
    def parse(self):
        ...
        self.op = WhereOp(
            token_id=0, token=tok, query=self.query, params=self.query.params
        )
...
class InnerJoinConverter(JoinConverter):
    ...
    def to_mongo(self):
        ...
        lookup = self._lookup()
        pipeline = [
            {"$match": {match_field: {"$ne": None, "$exists": True}}},
            lookup,
...
class OuterJoinConverter(JoinConverter):
    ...
    def to_mongo(self):
        ...
        pipeline = [
            ...
            {
                "$addFields": {
                    self.right_table: {"$ifNull": ["$" + self.right_table, null_fields]}
                }
            },
        ]
...
class OrderConverter(Converter):
    ...
    def parse(self):
        ...
        if isinstance(tok, Identifier):
            self.columns.append(
                (
                    SQLToken(tok[0], self.query.alias2op),
                    SQLToken(tok, self.query.alias2op),
                )
            )
            ...
            for _id in tok.get_identifiers():
                self.columns.append(
                    (
                        SQLToken(_id[0], self.query.alias2op),
                        SQLToken(_id, self.query.alias2op),
                    )
                )
    ...
    def to_mongo(self):
        return {
            "update": {
                "$set": {
                    sql.lhs_column: (
                        self.query.params[sql.rhs_indexes]
                        if sql.rhs_indexes is not None
                        else None
                    )
    ...
    def to_mongo(self):
        ...
        for selected in self.sql_tokens:
            if selected.table == self.query.left_table:
                ...
                try:
                    _id[selected.table][selected.column] = (
                        "$" + selected.table + "." + selected.column
                    )
                except KeyError:
                    _id[selected.table] = {
                        selected.column: "$" + selected.table + "." + selected.column
                    }

        return [{"$group": {"_id": _id}}, {"$replaceRoot": {"newRoot": "$_id"}}]
                ...
                "$addFields": {
                    "_nested_in": {
                        "$map": {
                            ...
                            "as": "lookup_result",
                            "in": "$$lookup_result."
                            + self._in_query.selected_columns.sql_tokens[0].column,
                        }
                    }
                }
            },
...
class HavingConverter(Converter):
    ...
    def parse(self):
        ...
        self.op = WhereOp(
            token_id=0, token=having, query=self.query, params=self.query.params
        )
        ...
        for iden in self.sql_tokens:
            if iden.table == self.query.left_table:
                ...
                try:
                    ...
                    _id[iden.table] = {
                        iden.column: "$" + iden.table + "." + iden.column
                    }
