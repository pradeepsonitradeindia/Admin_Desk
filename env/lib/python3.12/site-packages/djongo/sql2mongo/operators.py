...
class _InNotInOp(_IdentifierOp):
    ...
    def _to_mongo(self, op):
        if self.query.nested_query is not None:
            return {"$expr": {op: ["$" + self._field, "$_nested_in"]}}
...
class iLikeOp(LikeOp):
    def to_mongo(self):
        return {self._field: {"$regex": self._regex, "$options": "i"}}
...
class IsOp(_IdentifierOp):
    ...
    def to_mongo(self):
        ...
        return {self._field: None if is_null else {"$ne": None}}
...
class BetweenOp(_IdentifierOp):
    ...
    def to_mongo(self):
        if not self.is_negated:
            return {self._field: {"$gte": self._lower, "$lte": self._upper}}
        else:
            return {self._field: {"$not": {"$gte": self._lower, "$lte": self._upper}}}
...
class WhereOp(_Op):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not isinstance(self.token[2], Parenthesis):
            op = ParenthesisOp(
                0, sqlparse("(" + self.token.value[6:] + ")")[0][0], self.query
            )
        else:
    ...
    def __init__(self, *args, **kwargs):
        ...
        while tok_id:
            ...
            if tok.match(tokens.Keyword, "AND"):
                ...
                if (
                    tok[1].match(tokens.Name.Placeholder, ".*", regex=True)
