...
GSSAPIProperties = namedtuple(
    "GSSAPIProperties",
    ["service_name", "canonicalize_host_name", "service_realm", "service_host"],
)
...
def _validate_canonicalize_host_name(value: str | bool) -> str | bool:
    ...
    if value not in valid_names:
        raise ValueError(
            f"CANONICALIZE_HOST_NAME '{value}' not in valid options: {valid_names}"
        )
    return value
...
def _build_credentials_tuple(
    ...
    if mech == "GSSAPI":
        ...
        if properties.get("ALLOWED_HOSTS", None) is not None and human_callback is None:
            raise ConfigurationError(
                "ALLOWED_HOSTS is only valid with OIDC_HUMAN_CALLBACK"
            )
