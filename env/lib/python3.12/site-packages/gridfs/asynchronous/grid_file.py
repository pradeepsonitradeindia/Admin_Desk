...
class AsyncGridFS:
    ...
    def __init__(self, database: AsyncDatabase, collection: str = "fs"):
        ...
        if not isinstance(database, AsyncDatabase):
            raise TypeError(
                f"database must be an instance of Database, not {type(database)}"
            )
...
class AsyncGridFSBucket:
    ...
    def __init__(
        ...
        if not isinstance(db, AsyncDatabase):
            raise TypeError(
                f"database must be an instance of AsyncDatabase, not {type(db)}"
            )
    ...
    async def delete_by_name(
        ...
        if not res.deleted_count:
            raise NoFile(
                f"no file could be deleted because none matched filename {filename!r}"
            )
...
class AsyncGridOutCursor(AsyncCursor):
    ...
    async def next(self) -> AsyncGridOut:
        ...
        return AsyncGridOut(
            self._root_collection, file_document=next_file, session=self.session
        )
